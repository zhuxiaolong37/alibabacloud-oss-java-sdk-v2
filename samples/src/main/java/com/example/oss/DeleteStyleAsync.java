...
public class DeleteStyleAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient asyncClient = clientBuilder.build()) {
            ...
            CompletableFuture<DeleteStyleResult> future = asyncClient.deleteStyleAsync(request);
            DeleteStyleResult result = future.get();

            System.out.printf("Status code: %d%n", result.statusCode());
            System.out.printf("Request ID: %s%n", result.requestId());
            System.out.println("Delete style successfully.");
        } catch (Exception e) {
